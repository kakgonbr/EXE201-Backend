...
GO IF EXISTS (
        ...
        FROM sys.databases
        WHERE name = 'EXE-HH'
    ) BEGIN ALTER DATABASE [EXE-HH]
SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
DROP DATABASE [EXE-HH]
END;
GO CREATE DATABASE [EXE-HH]
GO USE [EXE-HH]
GO CREATE TABLE Users (
        ...
        CONSTRAINT ck_workshoplevel_name CHECK (
            Name IN ('Elementary', 'Intermediate', 'Advanced')
        )
        ...
        CONSTRAINT ck_workshop_status CHECK (
            Status IN (
                'draft',
                'awaiting',
                'verified',
        ...
        Repeats varchar(256) NOT NULL,
        -- "mon,fri" / "3,8,21" / "1/1,5/6,20/12"
        ...
        UserId int NOT NULL,
        WorkshopId int NOT NULL CONSTRAINT pk_workshoplike PRIMARY KEY (UserId, WorkshopId),
        ...
        CONSTRAINT ck_payment_amount CHECK (Amount > 0) -- CONSTRAINT fk_payment_workshopparticipant FOREIGN KEY (ParticipantId, TicketId) REFERENCES WorkshopParticipants
...
VALUES (
        'staff1@example.com',
        '',
        'AQAAAAIAAYagAAAAEC+EIwIzuiMSML2WhOWrh4RXtiohR0R6er4bsOUj1LKoNLrk4iRoSc6ah3qLgBcHtQ==',
        'staff',
        'staff1',
        1,
        1,
...
INSERT INTO WorkshopLevels (Name)
VALUES ('Elementary');
IF NOT EXISTS (
    SELECT 1
    FROM WorkshopLevels
...
INSERT INTO WorkshopCategories (Name)
VALUES ('Programming');
IF NOT EXISTS (
    SELECT 1
    FROM WorkshopCategories
...
DECLARE @t_w1_s1_morning INT,
    @t_w1_s1_afternoon INT,
    @t_w1_s1_evening INT,
    @t_w1_s2_morning INT,
    @t_w1_s2_evening INT,
    @t_w2_s3_morning INT,
    @t_w2_s3_afternoon INT,
    @t_w2_s4_evening INT;
INSERT INTO WorkshopTickets (
        TicketType,
        StartTime,
        EndTime,
        WorkshopScheduleId,
...
DECLARE @maybe_user1 INT,
    @maybe_user2 INT,
    @maybe_host1 INT,
    @maybe_host2 INT;
SELECT @maybe_user1 = Id
FROM Users
WHERE Email = 'user1@example.com';
SELECT @maybe_user2 = Id
FROM Users
WHERE Email = 'user2@example.com';
SELECT @maybe_host1 = Id
FROM Users
WHERE Email = 'host1@example.com';
SELECT @maybe_host2 = Id
FROM Users
WHERE Email = 'host2@example.com';
...
VALUES (
        @wk3,
        CAST(DATEADD(day, 40, GETDATE()) AS date),
        0
    );
...
-- Add workshop images for variety
-- NOTE: UX Design Crash (@wk7) intentionally has a schedule but NO tickets inserted so it will be excluded by search initial WHERE
VALUES (@wk4, 'https://via.placeholder.com/400');
-- Add workshop images for variety
VALUES (@wk5, 'https://via.placeholder.com/400');
INSERT INTO WorkshopImages (WorkshopId, ImgLink)
VALUES (@wk6, 'https://via.placeholder.com/400');
-- Add reviews to test Rating sort (ratings are ints; average will be used)
INSERT INTO WorkshopReviews (WorkshopId, UserId, Title, Description, Rating) -- Add reviews to test Rating sort (ratings are ints; average will be used)
INSERT INTO WorkshopReviews (WorkshopId, UserId, Title, Description, Rating)
VALUES (
        -- Add reviews to test Rating sort (ratings are ints; average will be used)
        INSERT INTO WorkshopReviews (WorkshopId, UserId, Title, Description, Rating) 'Excellent deep-dive',
            'Detailed and practical profiling tips.',
            5
        INSERT INTO WorkshopReviews (WorkshopId, UserId, Title, Description, Rating)
        INSERT INTO WorkshopReviews (WorkshopId, UserId, Title, Description, Rating)
        VALUES (
                ...
                @maybe_user2,
                'Very technical',
                'Great content but intense for some.',
                4
                INSERT INTO WorkshopReviews (WorkshopId, UserId, Title, Description, Rating)
                INSERT INTO WorkshopReviews (WorkshopId, UserId, Title, Description, Rating)
                VALUES (
                        ...
                        INSERT INTO WorkshopReviews (WorkshopId, UserId, Title, Description, Rating)
                        INSERT INTO WorkshopReviews (WorkshopId, UserId, Title, Description, Rating)
                        INSERT INTO WorkshopReviews (WorkshopId, UserId, Title, Description, Rating) 5 -- Add likes (WorkshopLikes) to test inclusion of Users navigation if needed in UI
                            -- Add likes (WorkshopLikes) to test inclusion of Users navigation if needed in UI
                        INSERT INTO WorkshopLikes (UserId, WorkshopId) -- Add likes (WorkshopLikes) to test inclusion of Users navigation if needed in UI
                        INSERT INTO WorkshopLikes (UserId, WorkshopId)
                        VALUES (@maybe_user2, @wk6);
-- Add likes (WorkshopLikes) to test inclusion of Users navigation if needed in UI
INSERT INTO WorkshopParticipants (ParticipantId, TicketId, Status) -- Add participants and payments to create more realistic dataset
INSERT INTO WorkshopParticipants (ParticipantId, TicketId, Status)
